...
typedef double creal_t; //main creal data type
typedef long double c_time; // this is the creal time type
 ...
 c_time crealTime, timeIncrementer; //these are used for keeping up with time 
 uint numOfRange; 
 c_time count; //the counting variable 
...
struct pre_actionlist crealActions[1000];
 ...
 void printOutDataInCrealNodes(struct crealNodeStruct nodes[]); //this will be used to print out all of the junk
  ...
  static inline int processProperties(creal_t returnVector[], creal_t properties[]); //process the properties
...
static inline int processSelectedActionsAndConditions(struct crealNodeStruct nodes[], creal_t properties[], creal_t returnVector[], uint rangePointer[], uint rangeSize); //this is the main function to process the creal nodes... through a selection!
...
/*  
  int writeOutDataFileFor(struct crealNodeStruct nodes[], uint rangePointer[], uint rangeIndex, c_time *currenTime);
  int transferDataFileDataFileIndex(struct crealNodeStruct *node, uint nodeIndex, c_time *currenTime); 
  int processProperties(creal_t returnVector[], creal_t properties[]); //process the properties
  int writeSelectedDataFile(struct crealNodeStruct *node, uint nodeIndex, uint propIndex, c_time *currentTime);
  */
  ...
  /* new functions to add to cerebro */
  FILE ***returnFileStructures(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[]);
  void freeFileArray(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[], FILE ***fileArray);
  int createFiles(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[], FILE ***fileArray);
static inline  int writeSelectedDataFile(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[], c_time *currentTime, FILE ***fileArray);
  int setStackSize(uint num); //this will change the stack size during runtime
  
  //~ uint findHighestPropertyValue(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[]);
...
int main(int argc, char **argv) 
 {
				...
				setStackSize(64); //set the stack size to 64MB
 				//The ranges, add this into cerebro
 				uint *rangeArray, range, highestAmountOfProp; //for the ranges
				...
				//~ FILE *fdArray = (FILE *) alloca(sizeof(FILE) * range); //this right here is going to be used for file descriptors
		...
		FILE ***mainFileArray; //  = returnFilePointersForNodes(rangeArray, range, crealNodes);
		...
		mainFileArray = returnFileStructures(rangeArray, range, crealNodes); //this is multithreaded
		
		createFiles(rangeArray, range, crealNodes, mainFileArray); //create the files
			
		//~ help = createFiles(rangeArray, range, crealNodes, 0, mainFileArray);
	...
	#pragma omp parallel default(shared) private(count)
	{
			...
			for (count = 0; count < crealTime; count++)
			{
				...
				#pragma omp sections nowait
				{
					...
					writeSelectedDataFile(rangeArray, range, crealNodes, &count, mainFileArray);				 
			...
			closeFiles(rangeArray, range, crealNodes, mainFileArray);//close all of the files
 
			freeFileArray(rangeArray, range, crealNodes, mainFileArray);//free the allocated memory for file
...
creal_t actionFunction_367_0(void)
{
	//this is where the time node is..
 ...
 FILE ***returnFileStructures(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[])
{
	 ...
	 if (nodes == NULL) { return NULL;} //just for safety
	 
	 	uint numProp, i, isNull;
		
		isNull = 0;
		
		FILE ***mainFileArray = malloc(sizeof(FILE *) * rangeSize);
		
		if (mainFileArray == NULL) { return NULL;} //add this here to safe guard the function
		
		//~ #pragma omp for ordered private(i,numProp) //openMP parallel for loop
	 	for(i = 0; i < rangeSize; i++)
	 	{
				

				numProp = findNumOfProperties(&nodes[rangePointer[i]]); //find the number of properties
				
				mainFileArray[i] = malloc(sizeof(FILE ) * numProp);
				
				if ( mainFileArray[i] == NULL) {perror("We ran out of memory!"); isNull = 1;}
				
		}
		//This condition is added for openmp
	 if (isNull > 0)
	 {
...
void freeFileArray(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[], FILE ***fileArray)
{
	 //this function is used to free the memories allocated for file streaming
	 if (nodes == NULL) { return;} //just for safety
		...
		//~ #pragma omp for ordered private(i) //openMP parallel for loop
	 	for(i = 0; i < rangeSize; i++)
	 	{
				//~ numProp = findNumOfProperties(&nodes[rangePointer[i]]); //find the number of properties		
				free(fileArray[i]);
 ...
 int createFiles(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[], FILE ***fileArray)
{
	 ...
	 if (nodes == NULL) { return -1;} //just for safety
	 
	 	uint numProp, i, isNull,k;
		
		isNull = 0;
		char path[1024]; // Good.... maybe the size should be increased eventually
		
		//~ *fileStreams = (FILE ***) malloc(rangeSize * sizeof(FILE **));
		
		//~ memset(*fileStreams,0,sizeof(FILE **) * rangeSize); //set everything to zero!!
	 	
		
		
		//~ #pragma omp for private(k,i,numProp,path) //openMP parallel for loop
	 	for(i = 0; i < rangeSize; i++)
	 	{
				...
				for(k=0; k< numProp; k++)
				{
					...
					sprintf(path,"%d_%d.dat\0", rangePointer[i], k);
					
					fileArray[i][k] = fopen(path, "w"); //MAKE SURE TO OPEN THIS THING UP FOR APPENDING
					
							if (fileArray[i][k] == NULL)
							{
	 ...
	 {
		return 0; //this is the default action going right here
	 }	
	 
}

int closeFiles(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[], FILE ***fileArray)
{
	 ...
	 if (nodes == NULL) { return -1;} //just for safety
		...
		//~ #pragma omp for private(k,i,numProp) //openMP parallel for loop
	 	...
	 	for(i = 0; i < rangeSize; i++)
	 	{
				...
				for(k=0; k< numProp; k++)
				{
					...
					fclose(fileArray[i][k]); //MAKE SURE TO OPEN THIS THING UP FOR APPENDING
...
static inline int writeSelectedDataFile(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[], c_time *currentTime, FILE ***fileArray)
{
	 ...
	 if (nodes == NULL) { return -1;} //just for safety
		...
		#pragma omp for private(k,i,numProp) nowait //openMP parallel for loop
	 	...
	 	for(i = 0; i < rangeSize; i++)
	 	{
				...
				for(k=0; k< numProp; k++)
				{
					...
					fprintf(fileArray[i][k],"%d %f\n",(uint) *currentTime, *(nodes[rangePointer[i]].properties[k]) ); //use fprintf to write directly to the file so we can write everything out to a file
					//~ fputs("HELLO!",fileArray[i][k]);
				}
...
uint findHighestPropertyValue(uint rangePointer[], uint rangeSize, struct crealNodeStruct nodes[])
{
	uint numProp, i, endResult;
	numProp = i = endResult = 0;
	
	for(i = 0; i < numProp; i++)
	{
		
		numProp = findNumOfProperties(&nodes[rangePointer[i]]); //find the number of properties
		if (numProp > endResult)
		{
			endResult = numProp;
...
{
	//this function is used to set the stack size of the application during runtime
	const rlim_t kStackSize = num * 1024 * 1024;   // min stack size = 16 MB ... you are going to want to put this at 64MB
    struct rlimit rl;
    int result;
...
static inline int processSelectedActionsAndConditions(struct crealNodeStruct nodes[], creal_t properties[], creal_t returnVector[], uint rangePointer[], uint rangeSize)
{
	...
	i = k = 0; //set to zero
	//~ memset(returnVector,0,1000*sizeof(creal_t)); //set the memory to zero	
	...
	//~ #pragma omp parallel
	//~ {
	//~ #pragma omp parallel for private(i,k,actionTruFalseArray)
	for (i = 0; i < rangeSize; i++)
	{
		...
		#pragma omp for private(k) nowait //openMP parallel for loop
		for (k = 0; k < nodes[rangePointer[i]].numberOfActions; k++)
		{
				if(actionTruFalseArray[k]) //if its true
				{
...
static inline int processProperties(creal_t returnVector[], creal_t properties[])
{
		...
		i = 0;
			...
			#pragma omp for private(i) //openMP parallel for loop
			for (i = 0; i < 1000; i++)
			{
...
/*	   
int writeOutDataFileFor(struct crealNodeStruct nodes[], uint rangePointer[], uint rangeIndex, c_time *currentTime)
{
	
		
	int i = 0;
	#pragma omp for private(i) nowait //openMP parallel for loop
...
int transferDataFileDataFileIndex(struct crealNodeStruct *node, uint nodeIndex, c_time *currentTime)
{
	int k = 0;
	
	uint numProp = findNumOfProperties(node); //find the number of properties
	
	//~ #pragma omp for private(k) //openMP parallel for loop
	for(k = 0; k < numProp; k++)
	{
